...
memory. `subtree_halo` walks the same boundary from outside as a
`NeighborCellIterator` — the level-`l` cells around the subtree, for
chunk-plus-halo reads — lazily on the aligned-block systems, by materialized
scan elsewhere. A5 is also the one system without `has_sorted_subtrees`, so

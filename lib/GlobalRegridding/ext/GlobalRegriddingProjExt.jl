...
const _TASK_CACHE_KEY = gensym(:GlobalRegriddingProjTaskCache)
...
function _task_cache()
    return get!(task_local_storage(), _TASK_CACHE_KEY) do
        IdDict{_ProjChartState,_ProjTaskPair}()
    end
end

function _task_pair(state::_ProjChartState; kwargs...)
    cache = _task_cache()
    pair = get(cache, state, nothing)

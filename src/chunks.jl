...
Apply `f` to each cell of `A` and its neighbourhood, chunk by chunk, writing
one result per cell into `dest`.

`neighborhood` is [`mapneighbors`](@ref)'s selector, [`Disc`](@ref)`(k)` or
[`Ring`](@ref)`(k)`. `halo` defaults to the selector's radius `k` (one ring at
least, which is the narrowest plan there is), and a caller may widen it. A
plan passed in must carry at least `k` rings: one built narrower is an
`ArgumentError` naming the `halo` to build it with.
...
The result is the whole-axis sweep's, cell for cell, in either form. A chunk's
halo of width `k` carries every axis cell within `k` steps of every cell the
chunk owns, so a neighbourhood computed on a chunk is the one computed on the
axis — clipped identically, and in the same order.

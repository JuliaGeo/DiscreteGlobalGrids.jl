...
The product of [`adjacency`](@ref): `t[p]` is the one-ring of in-region local
index `p` as a non-allocating `view`, counter-clockwise, and `length(t)` is the region
...
[`haloindices`](@ref) and [`halocells`](@ref) give the buffer's second half.
"""
...
The second half of the buffer `t`'s entries address, as complete-level global
indices or as cell ids: slot `length(t) + j` of a row is element `j` of these,
which is element `j` of [`halo`](@ref)`(region)` under the table's
connectivity.
...
resolves ids from the indices on each call.
"""
haloindices(t::AdjacencyTable) = t.halo
...
# An index is one-based, so `0` is free as the out-of-region sentinel and the
...
@inline function _adj_push!(loc::Vector{Int}, b::BufferedRows, q::Int, p::Int,
        c, nb)
    if q == 0
        j = Helpers.sorted_index(b.indices, p)
...
@noinline _adjacency_deep(n) = throw(ArgumentError(
    ...
    "local index, so a receptive field wider than one ring is a wider REGION: " *
...
@noinline _adjacency_unsorted() = throw(ArgumentError(
    "adjacency: the halo indices must be strictly ascending. That is the " *
...
@noinline _adjacency_lazy() = throw(ArgumentError(
    ...
    "ascending indices, not the lazy walk. `collect(halo(region; " *
    "connectivity))` is that vector, and `adjacency(region; halo = 1)` walks " *
    "it for you"))

@noinline _adjacency_incomplete(c, nb, p) = throw(ArgumentError(
    "adjacency: $c has neighbour $nb at global index $p, which is in neither " *
    "the region nor the halo, so its row cannot be completed. The indices " *
...
function adjacency(region::Region; halo::Union{Integer,Symbol} = 0,
    ...
    return adjacency(region, _halo_indices(region, connectivity);
...
adjacency(::Region, ::Union{SubtreeHaloIterator,SubsetHaloIterator,
    HaloIndexIterator}; kw...) = _adjacency_lazy()

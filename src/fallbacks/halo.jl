...
registered system inherits: the two level errors, then the one-ring at depth
zero, then [`OutsideWalkEngine`](@ref) — or [`ScanHaloEngine`](@ref) on a system
with no [`descendant_range`](@ref) to prune by.

...
# at two values of `N`: the same six-band table, the same tile lattice, and the same
# anchor for their pixel CENTRES.
...
sphere. `"one lattice nests k-fold inside another"` in
`test/systems/CopernicusDEM/runtests.jl` sweeps that co-location as `worst_post` and
asserts it below 1e-12 degrees; a half-pixel registration slip would move a post by
4.2e-4 degrees (1.5″). The `k²` fine posts of a block are the coarse post and the
...
!!! warning "The cell BOXES do not tile the coarse box"
    ...
    no single figure states it. At `k = 3`, in longitude it is 1.0″ in the `[0, 50)`
    band, 1.5″ in `[50, 60)` and 10″ in `[85, 90)`; in latitude it is 1.0″ in every
    band, because `Δlat` does not vary by band. The test suite pins the fraction rather
    than the arcseconds, as `worst_shift`: the west and north edges against
    `(1 - 1/k)/2`, asserted below 1e-10.

    The fine areas sum to that translated box (asserted below 1e-12 relative, as
    `worst_union`), and so miss the coarse cell's own area by about `tan(φ)·1″`, which
    the suite logs per tile row as `gap_lat50` and `gap_equator`.
    ...
    system's OWN pixel. Every gap quoted here is the relative gap
    `|block - coarse| / coarse`, and in those two rows it is of order one rather than
    of order `tan(φ)·1″`, logged as `worst_pole_n` and `worst_pole_s`. Their posts
    still coincide; nothing else about them does.
...
!!! warning "A grid hierarchy, not a value hierarchy"
    ...
    3x3 mean of public GLO-30, so co-located posts of the two products carry different
    elevations. Use this pair to relate CELLS; aggregate VALUES yourself if you need
    them consistent.
"""
function refine(coarse::CopernicusDEMSystem, fine::CopernicusDEMSystem,
    ...
    # rather than returned as-is: the relation being computed is between tile CORNERS,
    # and it is the identity on ids only because both lattices number tiles alike.

...
#
# `SquareRimEngine` descends the quadtree in Morton order and prunes quadrants
# that inherit none of the square's exposed sides. It emits ascending ids in
# `O(rim)` time and `O(Δ)` memory.
...
#
# `morton_to_xyd` of the block's first id names its minimum lattice corner.
# `side == 1` is depth zero, which the generic engine answers with
...
# Square-walk hooks. Morton orientation is `0x0` on every diamond.

...
#       the SET and the ORDER against an independent enumeration. Since the
#       order became a documented CONTRACT rather than a property, "halo_positions
#       is the ascending position stream, on every engine" restates it in the
#       space a caller consumes it in and pins it across all seven engines at
#       once, and `halo_positions`'s per-engine conversion with it.
#
#   An approximate size where an exact one is refused.
#       "halo_sizehint bounds the walk, and stays out of the count", which
#       derives every number in `halo_sizehint`'s docstring, and "the size hint
#       is what preallocates the collect", which measures what it buys. The
#       count contract itself is untouched — the last arm of the first testset
#       is what says so.
#
